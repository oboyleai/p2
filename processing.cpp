...
{
  const int currentWidth = Image_width(img);
  for (int i = 0; i < currentWidth - newWidth; i++)
  {
    Matrix *energy = new Matrix, *cost = new Matrix;
    const int seamLength = MAX_MATRIX_HEIGHT * MAX_MATRIX_WIDTH;
    int seam[seamLength];
